...
def get_object(object_type: str, name: str) -> str:
    ...
    Для каждого реквизита [SQL: SPxxx] — это SQL-имя поля, ВЫЧИСЛЕННОЕ по
    соглашению именования 1С 7.7 из внутреннего ID реквизита, а не факт из
    1Cv7.MD; не гарантировано на 100%. Это отдельное понятие от ID формы
    (в разделе "Формы") и от id_элемента в list_ert_dialog_controls — все
    три вида ID независимы друг от друга.
...
def resolve_id(object_id: str) -> str:
    ...
    Резолвит ID объектов метаданных (справочников, документов, регистров,
    реквизитов, форм и т.д.) из 1Cv7.MD. НЕ подходит для id_элемента формы
    из list_ert_dialog_controls/add_ert_dialog_control — тот ID локален для
    конкретной формы и не резолвится глобально.
...
def list_ert_dialog_controls(name: str) -> str:
    ...
    внешней обработки — id_элемента, класс контрола (BUTTON/STATIC/1CEDIT/
    BMASKED/...), подпись, координаты/размер, привязанный реквизит и его
    тип, действие (обработчик). Используйте перед add_ert_dialog_control/
    update_ert_dialog_control/remove_ert_dialog_control, чтобы узнать
    id_элемента существующих элементов и не пересекаться с ними по
    координатам. Работает для ЛЮБОЙ обработки (не только из --edit-path) —
    это инструмент чтения. Для сырого текста формы (без разбора) используйте
    get_ert_form.

    id_элемента — это локальный идентификатор, уникальный только в пределах
    ОДНОЙ формы. Он НЕ является SQL-полем и НЕ совпадает с ID реквизита
    объекта (bound_attribute — это имя реквизита, а не его ID) и не
    совпадает с ID формы как объекта метаданных (см. resolve_id).
...
def add_ert_dialog_control(
    ...
    координаты и существующие id_элемента (локальные для этой формы,
    не SQL-поля и не ID реквизита объекта).
...
def update_ert_dialog_control(
    ...
    прежними. id_элемента можно узнать через list_ert_dialog_controls.

    Args:
        name: Имя обработки без расширения .ert.
        control_id: id_элемента изменяемого элемента управления (локальный
            ID в пределах этой формы, НЕ SQL-поле и не ID реквизита объекта).
...
def remove_ert_dialog_control(name: str, control_id: int) -> str:
    ...
    Args:
        name: Имя обработки без расширения .ert.
        control_id: id_элемента удаляемого элемента управления (локальный
            ID в пределах этой формы, НЕ SQL-поле; см. list_ert_dialog_controls).
    """
